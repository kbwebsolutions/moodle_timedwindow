...
class condition extends \core_availability\condition {
     ...
     * Saves the restriction in the course_modules table availability column
     */
     ...
     * @param int $time ime in minutes
     * @return stdClass Object representing condition
     */
    public static function get_json($cmid, $time) : stdClass{
        $result = (object) [
          'type' => 'timewindow',
          'cm_id' => $cmid,
          'time' => (int)$time,
    ...
    public function is_available($not, info $info, $grabthelot, $userid): bool {
        ...
        $allow = $this->is_after_window($completiondata);

        return $allow;

    }
    /**
     * Check if the time now is after the
     * timewindow, meaning the item
     * will be made unavailable again
     *
     * @param stdClass $completiondata
     * @return bool
     */
    public function is_after_window($completiondata) : bool {
        $timeinseconds = $this->time * 60;
        $windowend = $completiondata->timemodified + $timeinseconds;
     ...
     */
    public function get_time(info $info): array {
    ...
    private function load_course_structure(stdClass $course): void {
        ...
        if (empty($this->modfastprevious)) {
            $previouscache = cache::make('availability_timewindow', 'previous_cache');
    ...
    /**
     * Get the description that will appear on the course when
     * the condition means an item is not available
     * @param bool $full
     * @param bool $not
     * @param info $info
     * @return string
    ...
    public function get_description($full, $not, \core_availability\info $info) {
        ...
        if ($this->cmid == self::OPTION_PREVIOUS && $editing) {
            ...
            $modinfo = $info->get_modinfo();
            if (!array_key_exists($cmid, $modinfo->cms) || $modinfo->cms[$cmid]->deletioninprogress) {
                $modname = get_string('missing', 'availability_timewindow');
            } else {
                $modname = '<AVAILABILITY_CMNAME_' . $modinfo->cms[$cmid]->id . '/>';
            }
        }

        $params = (object) ['modname' => $modname, 'time'=>$this->time];
        return get_string('status','availability_timewindow', $params);
    }
    /**
     * Obtains a representation of the options of this condition as a string,
     * i.e. what was entered into the js form fields when editing
     * Normally used for unit testing and by developers for debugging
     *
     * @return string Text representation of parameters
     */
    protected function get_debug_string() {
        return $this->time. ' minutes, module '. $this->cmid;
    }
    ...
    public static function completion_value_used($course, $cmid): bool {
        ...
        if (!array_key_exists($course->id, self::$modsusedincondition)) {
            ...
            foreach ($modinfo->cms as $othercm) {
                ...
                foreach ($tree->get_all_children('availability_timewindow\condition') as $cond) {
